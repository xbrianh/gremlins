...
- `state.py` — removed; `State`/`StateData` + `state.json` I/O moved to the
  Rust `_gremlins_core.executor` (`resolve_state_file`, `locked_update`).
...
(`launcher` → `spawn.pipeline` → `Gremlin`) and future SDK callers.
