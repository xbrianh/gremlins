...
def run_review(
    session_dir: pathlib.Path, plan_text: str, detail: str, client: Client
) -> int:
    is_git = in_git_repo()
    if is_git:
    ...
    state = State(client=client, session_dir=session_dir, gr_id=None, is_git=is_git)
    if plan_text:
    ...
    stage = review_code.ReviewCode(
        rc_entry.name, detail, rc_entry.prompts, rc_entry.options
    )
...
def run_address(session_dir: pathlib.Path, address: str, client: Client) -> int:
    ...
    stage = address_code.AddressCode(
        ac_entry.name, address, ac_entry.prompts, ac_entry.options
    )
    stage.run(state)
    return 0
