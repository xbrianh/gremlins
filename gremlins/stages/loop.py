...
def head_stable(state: State, iteration: int, head_before: str) -> bool:
    ...
    return _git.head_sha(pathlib.Path(state.artifacts.read("cwd"))) == head_before
...
class LoopStage(Stage):
    ...
    async def run(self, state: State) -> Outcome:
        for iteration in range(1, self._max_iterations + 1):
            ...
            head_before = _git.head_sha(pathlib.Path(iter_state.artifacts.read("cwd")))
...
def detach_to_pr_base(state: State) -> None:
    ...
    _git.git_detach_to_branch(branch, cwd=pathlib.Path(state.artifacts.read("cwd")))
