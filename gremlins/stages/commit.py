...
class Commit(Stage):
    def __init__(
        ...
        *,
        ...
        impl_handoff_branch: str | None = None,
    ...
    def _resolve_inputs(
        ...
        impl_handoff_branch = self.impl_handoff_branch or _read_state(
            sf, "impl_handoff_branch"
        )
        ...
        if impl_handoff_branch:
            try:
                commit_count = rev_list_count(f"{base_ref}..{impl_handoff_branch}")
        ...
        return (impl_outcome, impl_handoff_branch, base_ref, issue_url)

    def run(self, pipe: Any) -> None:
        if self.impl_outcome is None:
            impl_outcome, impl_handoff_branch, base_ref, issue_url = (
                self._resolve_inputs()
            )
        else:
            impl_outcome = self.impl_outcome
            impl_handoff_branch = self.impl_handoff_branch or ""
        ...
        diff = _get_diff(impl_outcome, impl_handoff_branch, base_ref, cwd_arg)
