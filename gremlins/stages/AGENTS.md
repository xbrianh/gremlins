...
- `sequence.py` — `SequenceStage(Stage)`. Runs `body: list[Stage]` sequentially in order, inheriting parent state (no fan-out). Child stages share artifacts and execution scope with the parent; state is passed through unchanged. Useful for bundling multi-stage units (e.g., the `handoff` recipe) that should appear as a single iteration in a parent loop. Stage type `"sequence"`.
