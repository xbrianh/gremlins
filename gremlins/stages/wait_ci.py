...
def _poll_until_done(
    ...
    """Poll PR checks until all are complete or timeout. Returns final checks list."""
    deadline = time.time() + timeout
    while True:
        if checks_getter is not None:
            checks, _ = checks_getter()
        else:
            checks = get_pr_ci_status(pr_url)["checks"]
