...
Drives launch_main() from gremlins.cli.launch with:
...
def test_launch_returns_gremlin_id(lenv):
    """launch_main() with --gremlin-id produces a well-formed GREMLIN_ID string."""
...
def test_launch_ghgremlin_persists_pipeline_default_client(lenv_with_gh):
    ...
    rc = launch_main([
        "gh",
        "--instructions", "test gh default client",
        "--gremlin-id", gremlin_id,
    ])
    assert rc == 0
...
def test_full_localgremlin_pipeline(lenv, monkeypatch):
    ...
    rc = launch_main([
        "local",
        "--instructions", "test full pipeline",
        "--gremlin-id", gremlin_id,
    ])
    assert rc == 0
    state_dir = _gremlins_state_root(lenv) / gremlin_id
