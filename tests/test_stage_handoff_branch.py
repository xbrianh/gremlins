...
def _make_stage(tmp_path: pathlib.Path) -> tuple[HandoffBranch, StageContext]:
    entry = _make_entry()
...
def _pipe(pre_state: PreImplState = PRE_STATE) -> SimpleNamespace:
...
def test_none_pre_state_raises(tmp_path: pathlib.Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="rewind to implement"):
        stage.run(SimpleNamespace(impl_pre_state=None))
