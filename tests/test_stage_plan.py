...
def test_plan_source_issue_ref_local(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """plan_source=#42 with no repo fetches issue body and writes plan.md."""
    plan_md = tmp_path / "plan.md"

    monkeypatch.setattr(
        "gremlins.stages.plan.get_repo", lambda: "owner/repo"
    )
...
def test_plan_source_file_github(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(cmd: list[str], *_args: object, **_kwargs: object) -> _subprocess.CompletedProcess[str]:
        if cmd[0] == "gh" and "create" in cmd:
            return _subprocess.CompletedProcess(cmd, 0, stdout=issue_url + "\n", stderr="")
    ...
    stage = Plan(_entry(), None, plan_source=str(plan_src), plan_file=plan_md, repo="owner/repo")
    stage.bind(_ctx(tmp_path, client))
...
def test_plan_source_issue_ref_github(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fake_view_issue(_ref: str, _repo: str) -> dict[str, object]:
        return {"body": "# GH Plan\nDetails.", "url": issue_url, "number": 99, "title": "GH Plan"}

    monkeypatch.setattr("gremlins.stages.plan.parse_issue_ref", _fake_parse_issue_ref)
    monkeypatch.setattr("gremlins.stages.plan.view_issue", _fake_view_issue)

    stage = Plan(_entry(), None, plan_source="#99", plan_file=plan_md, repo="owner/repo")
    client = FakeClaudeClient(fixtures={})
    stage.bind(_ctx(tmp_path, client))
