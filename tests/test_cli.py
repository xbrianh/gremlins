...
def test_run_pipeline_valid_id_proceeds(tmp_path, monkeypatch):
    monkeypatch.setattr("gremlins.spawn.pipeline.run_pipeline", lambda *a, **kw: 0)
    monkeypatch.setattr(
...
def test_run_pipeline_forwards_gremlin_id_to_orchestrator(
    ...
    monkeypatch.setattr("gremlins.spawn.pipeline.run_pipeline", fake_run_pipeline)
    monkeypatch.setattr(
        "_gremlins_core.executor.StateData.write_terminal_state",
        lambda self, exit_code: None,
    )
...
def test_launch_unified_dispatch_calls_launch(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda name: pathlib.Path(f"/fake/{name}.yaml"),
    )
    monkeypatch.setattr(
...
def test_launch_list_prints_pipeline_names(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("gremlins.cli.launch.list_pipelines", lambda: fake_pipelines)
