...
def write_done_from_shell_cmd(cmd: str) -> None:
    ...
    Note: This parses any quoted path containing ``/done`` or ``_done`` from the
    shell command string. It is coupled to the convention that the verify
    recipe's shell wrapper writes ``done`` to ``{artifact_dir}/done``. If the
    wrapper format or path naming changes, this function silently returns
    without creating the file, causing the verify loop to time out in tests.
    """
    import re as _re
...
def common_local_patches(monkeypatch):
    ...
    monkeypatch.setattr("_gremlins_core.utils.proc.run_shell_async", _noop_shell)
