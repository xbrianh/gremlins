...
def test_head_stable_returns_true_when_head_unchanged(tmp_path):
    ...
    assert head_stable(_make_gremlin_wrapper(state), 1, "") is True


def test_head_stable_returns_false_when_head_changed(tmp_path):
    state = _loop_state(tmp_path)
    assert head_stable(_make_gremlin_wrapper(state), 1, "old-sha-abc123") is False


def test_max_iters_terminates_at_n(tmp_path):
    state = _loop_state(tmp_path)
    pred = max_iters(2)
    assert not pred(_make_gremlin_wrapper(state), 1, "")
    assert pred(_make_gremlin_wrapper(state), 2, "")
    assert pred(_make_gremlin_wrapper(state), 3, "")
