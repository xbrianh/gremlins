...
from gremlins.executor.state import GremlinWrapper, State as RuntimeState, StateData, build_state
...
def test_loop_head_stable_exits_cleanly(tmp_path):
    ...
    outcome = asyncio.run(loop.run(GremlinWrapper(state)))
...
def test_loop_cmd_failure_then_fix_then_green(tmp_path):
    ...
    asyncio.run(loop.run(GremlinWrapper(loop_state)))
...
def test_loop_fix_skipped_on_success(tmp_path):
    ...
    loop = LoopStage("loop", body_runners=[check, fix], max_iterations=3)
    state = _loop_state(tmp_path)
    asyncio.run(loop.run(GremlinWrapper(state)))
...
def test_custom_until_predicate(tmp_path):
    ...
    state = _loop_state(tmp_path)
    result = asyncio.run(loop.run(GremlinWrapper(state)))
    assert result == Done()
