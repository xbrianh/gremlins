...
def test_format_n(tmp_path: pathlib.Path):
    s = _make_state(
...
def test_format_attempt(tmp_path: pathlib.Path):
    s = _make_state(
        tmp_path, loop_iteration=3, attempt="abc", scope_names=["implement", "verify"]
    )
...
def test_format_scope(tmp_path: pathlib.Path):
    s = _make_state(
        tmp_path, loop_iteration=3, attempt="abc", scope_names=["implement", "verify"]
    )
...
def test_format_combined(tmp_path: pathlib.Path):
    s = _make_state(
        tmp_path, loop_iteration=3, attempt="abc", scope_names=["implement", "verify"]
    )
...
def test_format_empty_scope(tmp_path: pathlib.Path):
