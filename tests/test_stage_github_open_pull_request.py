...
def test_prompt_includes_explicit_push_and_pr_create(tmp_path):
    ...
    with patch("gremlins.stages.github_open_pull_request.run_agent", _fake_run_agent(prompts)):
        with patch("gremlins.stages.github_open_pull_request.extract_gh_url", return_value=PR_URL):
...
def test_stacked_pr_uses_prev_branch_as_base(tmp_path):
    ...
    with patch("gremlins.stages.github_open_pull_request.run_agent", _fake_run_agent(prompts)):
        with patch("gremlins.stages.github_open_pull_request.extract_gh_url", return_value=PR_URL):
            asyncio.run(stage.run(_state(tmp_path, prev_pr_branch="feat-child-1", base_ref="main")))
    assert "feat-child-1" in prompts[0]
    assert "main" not in prompts[0].split("--base")[1] if "--base" in prompts[0] else True
