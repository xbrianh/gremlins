...
def test_review_main_requires_commit_diff_or_dirty_worktree(
    ...
    _common_patches(monkeypatch)
    monkeypatch.setattr("gremlins.orchestrators.review_address.in_git_repo", lambda: True)
    monkeypatch.setattr("gremlins.orchestrators.review_address.rev_exists", lambda rev: True)
    monkeypatch.setattr(
...
def test_review_main_allows_dirty_worktree_without_commit_diff(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gremlins.orchestrators.review_address.has_dirty_worktree", lambda: True)
